...
import { DuelApiService } from "src/blockchain/services/duel.js";

export class ClientPair implements ILogger {
  private _id: number;
  private _clients: Map<string, Client>;
  private _tutorComplete: Map<string, boolean>;
  ...
  private async addClient(aClient: Client) {
    const battleAcceptTimer = 15; // in sec
    // let showTutor = await DuelApiService.getInstance().getUserDuelCount(aClient.connectionId) <= 0;
    let showTutor = true;
    try {
      // await DuelApiService.getInstance().getUserDuelCount(aClient.gameData.id).then(n => showTutor = n <= 0);
    } catch (error) {
      this.logError(`getUserDuelCount:`, error);
    }
    this.logDebug(`showTutor: ${showTutor}`);
    this._clients.set(aClient.connectionId, aClient);
    aClient.onAcceptScreenPack.add(this.onAcceptScreenPack, this);
    aClient.sendAcceptScreenStart(battleAcceptTimer, showTutor);
  }
